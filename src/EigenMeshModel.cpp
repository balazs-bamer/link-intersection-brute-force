...
fragor::EigenMeshJointLink::EigenMeshJointLink(urdf::Model const &aModel,
  ...
  while(!links2process.empty()) {
    ...
    auto geomsh{coll->geometry}; // TODO coll is null sometimes
    auto geom = geomsh.get();
    auto mesh = dynamic_cast<urdf::Mesh*>(geom);
    if(mesh != nullptr) {
      Log::n() << mesh->filename << Log::end;
    }
    else {
      // TODO something should happen to links without a mesh.
    }
    for(auto &i : aLinkId2parentLinkId) {
      if(i.second == actualLinkId && remainingLinks.contains(i.first)) {
...
fragor::EigenMeshModel::EigenMeshModel(urdf::Model const &aModel, std::map<std::string, std::string> const &aParentLinkTree, std::unordered_set<std::string> const aForbiddenLinks) {
  ...
  while(!links2process.empty()) {
    ...
    do {
      wasIncrement = false;
      for (auto &i : linkId2parentLinkId) {                // O(n^2) but who cares? Not many links in an URDF.
        ...
        && !aForbiddenLinks.contains(id2name[i.first])) {  // first is child
          ...
          if(jointType == urdf::Joint::FIXED) {
            ...
            linkId2parentLinkId.emplace(i.first, actualLocalRootId);
          ...
          else { // Nothing to do, because we don't process anything else.
          }
        }
        else { // nothing to do
        }
